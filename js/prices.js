...
function loadAllSettings() {
    ...
    if (savedLights) {
        const parsed = JSON.parse(savedLights);
        for (let key in parsed) {
            if (window.LIGHT_DATA && window.LIGHT_DATA[key]) {
                window.LIGHT_DATA[key].price = parsed[key].price;
            }
        }
    }
    if (savedExtras) {
        ...
        for (let key in parsed) {
            if (window.EXTRA_DATA && window.EXTRA_DATA[key]) {
                window.EXTRA_DATA[key].price = parsed[key].price;
            }
        }
    }
    if (savedRails) {
        const parsed = JSON.parse(savedRails);
        for (let key in parsed) {
            if (window.RAIL_DATA && window.RAIL_DATA[key]) {
                window.RAIL_DATA[key].price = parsed[key].price;
            }
        }
    }
