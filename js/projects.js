        ...
        db.collection('users').doc(currentUser.uid).collection('projects').doc(projectId).get()
            .then((doc) => {
                if (doc.exists) {
                    ...
                    // Полное восстановление данных (включая материалы)
                    // project.data уже содержит все материалы, так как мы сохраняем полный rooms массив
                    rooms = JSON.parse(JSON.stringify(project.data));
                    
                    // Дополнительная проверка: если вдруг материалы не сохранились, 
                    // восстанавливаем структуру materials для каждой комнаты
                    rooms.forEach((room, idx) => {
                        ...
                        // Восстанавливаем wallProfiles как объект, если он был массивом или отсутствует
                        if (!room.materials.wallProfiles) {
                            room.materials.wallProfiles = {};
                        }
                        // Если wallProfiles был массивом, конвертируем в объект
                        if (Array.isArray(room.materials.wallProfiles)) {
                            const newProfiles = {};
                            room.materials.wallProfiles.forEach((profile, i) => {
                                if (profile) newProfiles[i] = profile;
                            });
                            room.materials.wallProfiles = newProfiles;
                        }
                    });
